    ...
    public partial class New_user : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (count == 0)
            {
                ...
                {
                    DialogResult result = MessageBox.Show("Некоторые или все данные кроме номера телефона состоят менее чем из 3 символов. В данном случае вы добавите только номер телефона. Если же вас устраивает сохранение одного номера - нажмите Да", " ", MessageBoxButtons.YesNo);
                ...
                else
                {
                    ...
                    if (number.Length > 9 && number.Length < 14 && name.Length >= 3 && surname.Length >= 3 && patronymic.Length >= 3 && address.Length >= 3)
                    {
                        DialogResult result = MessageBox.Show("Все данные введены корректно. В вашем случае вы сохраните полную информацию о пользователе. Если вас все устраивает - нажмите Да.", " ", MessageBoxButtons.YesNo);
                        if (result == DialogResult.Yes)
                        {
                            Database.size++;
                            Array.Resize(ref Database.phone_number, Database.size);
